...
public record SiteProductDto(long Id, string Name, string Handle, string? Description, SiteParentProductDto? Parent, SiteProductGroupDto? Group, string? Image, string Currency, decimal? Price, decimal? RegularPrice, int? Available, IEnumerable<YourBrand.Catalog.ProductAttributeDto> Attributes, IEnumerable<YourBrand.Catalog.ProductOptionDto> Options, bool HasVariants);
