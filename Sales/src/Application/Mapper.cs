...
{
    public static OrderDto ToDto(this Order order) => new OrderDto(order.Id, (OrderStatusDto)order.Status, order.AssignedTo?.ToDto(), order.Items.Select(x => x.ToDto()), order.Created, order.CreatedBy.ToDto(), order.LastModified, order.LastModifiedBy?.ToDto());
    ...
    public static CartItemDto ToDto(this CartItem cartItem) => new CartItemDto(cartItem.Id, cartItem.Description, cartItem.ItemId, cartItem.Price, cartItem.Quantity, cartItem.Total, cartItem.Created, cartItem.CreatedBy.ToDto(), cartItem.LastModified, cartItem.LastModifiedBy?.ToDto());
