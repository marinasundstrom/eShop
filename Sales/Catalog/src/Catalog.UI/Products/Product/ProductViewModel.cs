...
public class ProductViewModel
{
    ...
    public string Id => Product.Id.ToString(); //Product?.Handle ?? string.Empty;
    ...
    public async Task Initialize(string id, string? variantId)
    {
        ...
        if (Product.HasVariants)
        {
            ...
            {
                var variants = (await productsClient.GetVariantsAsync(Id, 0, 20, null, null, null)).Items;
            ...
            Variants.AddRange((await productsClient.FindVariantByAttributeValues2Async(Id, selectedAttributeValues)));
    ...
    public async Task SelectVariant(ProductDto variant)
    {
    ...
    public async Task UpdateVariant()
    {
        ...
        var products = await productsClient.FindVariantByAttributeValues2Async(Id, selectedAttributeValues);
        ...
        variant = await productsClient.FindVariantByAttributeValuesAsync(Id, selectedAttributes.ToDictionary(x => x.Id, x => x.SelectedValueId));
    ...
    public ProductDto? Variant { get => variant; set => variant = value; }
