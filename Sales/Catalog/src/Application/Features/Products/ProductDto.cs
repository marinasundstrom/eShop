...
public record class ProductDto(long Id, string Name, string Handle, string? SKU, string? Description, ParentProductDto? Parent, ProductGroupDto? Group, string? Image, decimal Price, decimal? RegularPrice, int? QuantityAvailable, bool HasVariants, ProductVisibility? Visibility, IEnumerable<ProductAttributeDto> Attributes, IEnumerable<ProductOptionDto> Options);
