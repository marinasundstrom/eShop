...
public record class ProductDto(long Id, string Name, string Handle, string? SKU, string? Description, ParentProductDto? Parent, ProductGroupDto? Group, string? Image, decimal Price, decimal? CompareAtPrice, int? QuantityAvailable, bool HasVariants, ProductVisibility? Visibility, IEnumerable<ProductAttributeDto> Attributes, IEnumerable<ProductOptionDto> Options);

public record class ParentProductDto(long Id, string Name, string Handle, string? SKU, string? Description, ProductGroupDto? Group);
